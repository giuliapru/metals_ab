...
snaps = np.array([30, 39, 51])#np.array([30, 39, 51, 68, 92, 129, 188]) #30 39 51, 68, 92, 129, 188
redshifts = np.array([9, 8, 7]) #np.array([9, 8, 7, 6, 5, 4, 3]) #8,7,6,5, 4
gal = ['g2']#['g5229300', 'g2274036', 'g519761', 'g500531', 'g137030', 'g37591','g33206', 'g10304', 'g5760', 'g1163', 'g578', 'g205', 'g39', 'g2']
density_cut = -2
...
for g in gal:
    for i in range(len(snaps)):
        # Example parameters
        job_name = "DLA_cut_%s_z%d" %(g, redshifts[i])
        output_file = "/home/gpruto/metal_ab/code/jobs/DLA_cut/DLA_cut_%s_z%d.out" %(g, redshifts[i]) # %j will be replaced with the job ID
        error_file = "/home/gpruto/metal_ab/code/jobs/DLA_cut/DLA_cut_%s_z%d.err" %(g, redshifts[i])
        times = "10:00:00"  if g=='g2' or g=='g39' else "02:00:00"
        nodes = 1
        mem_per_cpu = "100GB" if g == 'g39' or g=='g205' or g=='g578' else "200GB" if g=='g2' else "50GB"  # Adjust memory based on galaxy
        ntasks_per_node = 1
        job_script_name = "/home/gpruto/metal_ab/code/jobs/DLA_cut/DLA_cut_%s_z%d.slurm" %(g, redshifts[i])
        job_commands = "python /home/gpruto/metal_ab/code/DLA_cut/DLA_cut.py %d %s %.1f" %(snaps[i], g, density_cut) 
