...
figss, axss = plt.subplots(2, 2, figsize=(10, 10))
ax = ax.flatten()
axs = axs.flatten()
axss = axss.flatten()

for aa in [ax, axs, axss]:
...
for g in gal: 
    ...
    # if you can use files with ptcs in the CGM of target haloes already selected  
        ...
        with h5py.File(infile, 'r') as f:
            ...
            silicon_density = f['silicon_density'][:] * factor_mod_si
    ...
    ax[0].scatter([], [], s=7, label = r'%s, $M_h = %.2e$ M$_\odot$, $M_* = %.2e$ M$_\odot$, SFR$_{10} = %.2f$ M$_\odot$ yr$^{-1}$, SFR$_{100} = %.2f$ M$_\odot$ yr$^{-1}$' % (g, halo_mass, stellar_mass, sfr_10, sfr_100), color = colours[gal.index(g)])
    #axs[0].scatter([], [], s=7, label = r'%s, $M_h = %.2e$ M$_\odot$, $M_* = %.2e$ M$_\odot$, SFR$_{10} = %.2f$ M$_\odot$ yr$^{-1}$, SFR$_{100} = %.2f$ M$_\odot$ yr$^{-1}$' % (g, halo_mass, stellar_mass, sfr_10, sfr_100), color = colours[gal.index(g)])
    axss[0].scatter([], [], s=7, label = r'%s, $M_h = %.2e$ M$_\odot$, $M_* = %.2e$ M$_\odot$, SFR$_{10} = %.2f$ M$_\odot$ yr$^{-1}$, SFR$_{100} = %.2f$ M$_\odot$ yr$^{-1}$' % (g, halo_mass, stellar_mass, sfr_10, sfr_100), color = colours[gal.index(g)])
    ax[0].scatter(c_fe, o_fe, s=1, alpha=0.3, color = colours[gal.index(g)])
    mlib.add_contours(axss[0], c_fe, o_fe, colours[gal.index(g)])
    ax[1].scatter(c_fe, si_c, s=1, alpha=0.3, color = colours[gal.index(g)])
    mlib.add_contours(axss[1], c_fe, si_c, colours[gal.index(g)])
    ax[2].scatter(o_fe, si_fe, s=1, alpha=0.3, color = colours[gal.index(g)])
    mlib.add_contours(axss[2], o_fe, si_fe, colours[gal.index(g)])
    ax[3].scatter(c_o, si_o, s=1, alpha=0.3, color = colours[gal.index(g)])
    mlib.add_contours(axss[3], c_o, si_o, colours[gal.index(g)])

mlib.hist_2d(np.array(c_fe_allg), np.array(o_fe_allg), axs[0])
mlib.hist_2d(np.array(c_fe_allg), np.array(si_c_allg), axs[1])
mlib.hist_2d(np.array(o_fe_allg), np.array(si_fe_allg), axs[2])
mlib.hist_2d(np.array(c_o_allg), np.array(si_o_allg), axs[3])
...
handless, labelss = axss[0].get_legend_handles_labels()
figss.legend(handless, labelss, loc='lower center', ncol=2, fontsize=8)
fig.subplots_adjust(bottom=0.2)
fig.subplots_adjust(wspace=0.3, hspace=0.3)
figss.subplots_adjust(bottom=0.2)
figss.subplots_adjust(wspace=0.3, hspace=0.3)
figs.subplots_adjust(bottom=0.2)
figs.subplots_adjust(wspace=0.3, hspace=0.3)

if cold_gas_only:
    if factor_mod_si != 1:
        fig.savefig('/home/gpruto/CGM_ref_analysis/images/metals/target_haloes/test_CGM_only_z%.1f_coldgasonly_modsi.png' % red, bbox_inches='tight')
        figs.savefig('/home/gpruto/CGM_ref_analysis/images/metals/target_haloes/test_CGM_only_z%.1f_hist_coldgasonly_modsi.png' % red, bbox_inches='tight')
        figss.savefig('/home/gpruto/CGM_ref_analysis/images/metals/target_haloes/test_CGM_only_z%.1f_contours_coldgasonly_modsi.png' % red, bbox_inches='tight')
        plt.close()
    else:
        fig.savefig('/home/gpruto/CGM_ref_analysis/images/metals/target_haloes/test_CGM_only_z%.1f_coldgasonly.png' % red, bbox_inches='tight')
        figs.savefig('/home/gpruto/CGM_ref_analysis/images/metals/target_haloes/test_CGM_only_z%.1f_hist_coldgasonly.png' % red, bbox_inches='tight')
        figss.savefig('/home/gpruto/CGM_ref_analysis/images/metals/target_haloes/test_CGM_only_z%.1f_contours_coldgasonly.png' % red, bbox_inches='tight')
        plt.close()
            
         
else:
    if factor_mod_si != 1:
        fig.savefig('/home/gpruto/CGM_ref_analysis/images/metals/target_haloes/test_CGM_only_z%.1f_modsi.png' % red, bbox_inches='tight')
        figs.savefig('/home/gpruto/CGM_ref_analysis/images/metals/target_haloes/test_CGM_only_z%.1f_hist_modsi.png' % red, bbox_inches='tight')
        figss.savefig('/home/gpruto/CGM_ref_analysis/images/metals/target_haloes/test_CGM_only_z%.1f_contours_modsi.png' % red, bbox_inches='tight')
        plt.close()
    else:
        fig.savefig('/home/gpruto/CGM_ref_analysis/images/metals/target_haloes/test_CGM_only_z%.1f.png' % red, bbox_inches='tight')
        figs.savefig('/home/gpruto/CGM_ref_analysis/images/metals/target_haloes/test_CGM_only_z%.1f_hist.png' % red, bbox_inches='tight')
        figss.savefig('/home/gpruto/CGM_ref_analysis/images/metals/target_haloes/test_CGM_only_z%.1f_contours.png' % red, bbox_inches='tight')
        plt.close()
