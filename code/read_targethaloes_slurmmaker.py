...
for i in range(len(snaps)):
    # Example parameters
    job_name = "read_%d" %(redshifts[i])
    output_file = "/home/gpruto/CGM_ref_analysis/code/metals/jobs/read_target_z%d.out" %(redshifts[i]) # %j will be replaced with the job ID
    error_file = "/home/gpruto/CGM_ref_analysis/code/metals/jobs/read_target_z%d.err" %(redshifts[i])
    times = "72:00:00"  
    nodes = 1
    ntasks_per_node = 1
    job_script_name = "/home/gpruto/CGM_ref_analysis/code/metals/jobs/read_target_z%d.slurm" %(redshifts[i])
    job_commands = "python /home/gpruto/CGM_ref_analysis/code/metals/read_targethaloes.py %d" %(snaps[i]) 
